...
std::tuple<torch::Tensor, torch::Tensor>
CPURandomSampler::WeightedSample(const torch::Tensor& nodes, int32_t req_num) {
  if (req_num < 0) req_num = std::numeric_limits<int32_t>::max();
  const int64_t* nodes_ptr = nodes.data_ptr<int64_t>();
  int64_t bs = nodes.size(0);
  const auto row_ptr = graph_->GetRowPtr();
...
std::tuple<torch::Tensor, torch::Tensor, torch::Tensor>
CPURandomSampler::WeightedSampleWithEdge(
...
void CPURandomSampler::CSRRowWiseSample(
  ...
  at::parallel_for(0, bs, 1, [&](int32_t start, int32_t end){
    for(int32_t i = start; i < end; ++i) {
      auto v = nodes[i];
      if (v < row_count) {
        WeightedRandomSample(col_idx + row_ptr[v], col_idx + row_ptr[v+1], req_num,
...
void CPURandomSampler::CSRRowWiseSample(
  ...
  at::parallel_for(0, bs, 1, [&](int32_t start, int32_t end){
    for(int32_t i = start; i < end; ++i) {
      auto v = nodes[i];
      if (v < row_count) {
        WeightedRandomSample(col_idx + row_ptr[v], col_idx + row_ptr[v+1],
            edge_ids + row_ptr[v], edge_ids + row_ptr[v+1],
...
void CPURandomSampler::WeightedRandomSample(const int64_t* col_begin,
                                            const int64_t* col_end,
                                            const int32_t req_num,
