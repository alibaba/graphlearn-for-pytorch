...
class UnifiedTensorTestCase(unittest.TestCase):
  ...
  def _test_with_dtype(self, dtype: torch.dtype):
    ...
    cpu_tensor3 = cpu_tensor1 * 3

    tensors = [cpu_tensor1, cpu_tensor2, cpu_tensor3]
    tensor_devices = [0, 1, -1]

    unified_tensor = UnifiedTensor(0, dtype)
    unified_tensor.init_from(tensors, tensor_devices)
    self.assertEqual(unified_tensor.shape, [128*3, 128])
    ...
    expected_res = torch.cat((feature, feature*2, feature*3), 0)
